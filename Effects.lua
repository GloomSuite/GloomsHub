-- ============================================================
-- Effects.lua — Gloom's Hub: the suite's SHAPED ANIMATION engine.
--
-- Eight self-contained animation MODULES that render onto any host
-- frame, clipped to any silhouette from Shapes.lua. Gloom's Bars
-- drives them from action-button triggers (procs, casts, hover);
-- Gloom's Auras drives them from its own aura displays. Neither
-- owns them, so both get all eight and neither can drift.
--
-- ★ WHY THIS LIVES HERE. These modules were GB's Anims.lua until
-- 2026-08-25, and they were always written host-agnostic — the
-- original header notes Start/Stop are keyed by host frame, "a
-- button, OR the Config preview frame". That is exactly what made
-- them portable: the move rewired paths and nothing else. GB keeps
-- what is genuinely GB's — WHICH trigger runs WHICH animation, its
-- per-trigger data model, and the reconcile loop over its buttons.
--
-- MODULE CONTRACT (unchanged from GB's):
--   { id, label, defaults = {..}, params = { {key, kind, label, ...}, .. },
--     Start(host, icon, key, p), Stop(host) }
-- `host` is any frame; `icon` is the region the effect sizes itself
-- against; `key` is a Shapes.lua silhouette key. Start is idempotent
-- — call it repeatedly to reconfigure live; it reuses the host's
-- instance. Params `kind` is what a consumer's UI switches on:
-- "color" | "range" | "bispeed" | "choice".
--
-- ⚠ Every module attaches its clip mask through a DEFERRED
-- C_Timer.After(0), because AddMaskTexture silently fails on a
-- texture that has never rendered. Don't "simplify" that away; the
-- comment at PRIME_ALPHA explains the flash it prevents.
-- ============================================================

GloomsHub = GloomsHub or {}
_G.GloomsHub = GloomsHub

local Effects = { modules = {}, order = {} }
GloomsHub.Effects = Effects

-- Bumped when the module set or the contract changes, so a consumer can gate on
-- capability instead of guessing from a Hub version number.
Effects.VERSION = 1

-- Shared (non-per-shape) effect art. Per-shape masks come from GloomsHub:ShapeAsset.
local EFFECT_ART = "Interface\\AddOns\\GloomsHub\\Media\\art\\effects\\"

function Effects:Register(mod)
  if self.modules[mod.id] then return end
  self.modules[mod.id] = mod
  self.order[#self.order + 1] = mod.id
end
function Effects:Get(id) return self.modules[id] end
function Effects:Each(fn) for _, id in ipairs(self.order) do fn(self.modules[id]) end end

local function copyval(v) if type(v) == "table" then return { v[1], v[2], v[3], v[4] } else return v end end

-- A module's defaults with `saved` laid over the top, so the engine and every UI
-- always see a FULL param set. Colour defaults are copied, never shared by
-- reference — a consumer editing one would otherwise rewrite the module's default
-- for every host in the game. nil if the module id is unknown.
function Effects:MergeParams(id, saved)
...
function Shine:Start(host, icon, key, p)
  ...
  GloomsHub:GrowAnchor(inst.mask, icon, 0)
  inst.mask:SetTexture(GloomsHub:ShapeAsset(key, "rim"), "CLAMPTOBLACKADDITIVE", "CLAMPTOBLACKADDITIVE")
  for i = 1, n do
...
-- ===========================================================================
local MARCH_TEX = EFFECT_ART .. "march.png"
...
function March:Start(host, icon, key, p)
  ...
  local s = 1.6 * math.max(icon:GetWidth(), icon:GetHeight())   -- oversize so the spin covers the rim
  GloomsHub:GrowAnchor(inst.mask, icon, 0)
  inst.mask:SetTexture(GloomsHub:ShapeAsset(key, "line"), "CLAMPTOBLACKADDITIVE", "CLAMPTOBLACKADDITIVE")
  for i = 1, n do
    local tex = inst.texs[i]
    if not tex then
      tex = inst.frame:CreateTexture(nil, "OVERLAY")
...
-- ===========================================================================
local SHEEN_TEX = EFFECT_ART .. "sheen.png"
...
function Sheen:Start(host, icon, key, p)
  ...
  GloomsHub:GrowAnchor(inst.mask, icon, 0)
  inst.mask:SetTexture(GloomsHub:ShapeAsset(key, "base"), "CLAMPTOBLACKADDITIVE", "CLAMPTOBLACKADDITIVE")
...
-- ===========================================================================
local SPARKLE_TEX = EFFECT_ART .. "sparkle.png"
...
function Breathe:Start(host, icon, key, p)
  ...
  inst.peak = 1
  local color = p.color or { 1, 1, 1 }
  inst.tex:SetTexture(GloomsHub:ShapeAsset(key, "rim"))
  inst.tex:SetVertexColor(color[1], color[2], color[3])
...
function Burst:Start(host, icon, key, p)
  ...
  local rim = GloomsHub:ShapeAsset(key, "rim")
  for i = 1, n do
    local tex = inst.texs[i]
    if not tex then
      tex = inst.frame:CreateTexture(nil, "OVERLAY")
...
-- ===========================================================================
local RADAR_TEX = EFFECT_ART .. "radar.png"
local radarInst = {}
local radarActive = {}
