...
**Repo:** `~/GloomsAuras` · **Size:** one line · **Evidence:** `TESTED` (2026-07-25) · **live bug today**

`Displays.lua:379` assumes `SetFont` returns false on a bad asset. It raises instead, so the
fallback never runs and `ApplyConfig` aborts mid-function — the display breaks entirely, not just
its text. Fires whenever an aura's font points into an addon that isn't installed.

**Why it matters beyond the owner's machine:** his config references three external addons, so
**this will hit friends the first time the suite is shared.**

**Fix:** `pcall` the first `SetFont`, fall back on failure. Not yet written.
**Read first:** [FINDINGS.md](FINDINGS.md) §2 · `~/GloomsAuras/Displays.lua` around line 379

---

### 3 · MM Hunter auras never tested on the PTR
...
### 4 · The 12.1 exposure sweep is unfinished
...
- **Overlays logo placement inside the tab** — closed at Phase E gate B; a mark + wordmark in the
  rail, not a splash.
- **The 7-phase plan** — complete, QA'd, closed. No phase work remains.
