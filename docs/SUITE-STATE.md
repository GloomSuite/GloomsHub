...
| **G** | Packaging/release: `.pkgmeta`, `## Dependencies: GloomsHub`, embed LibGloomSkin, WoWup. | not started. ⚠⚠ **BLOCKING PREREQUISITE — IDENTITY SCRUB (2026-07-24).** The owner's real name + personal email were exposed on the then-public repos: 212 "the owner" mentions across 13 files in GloomsBars, 90 across 7 in GloomsAuras, plus 168 commits authored as `<gloom@handofdevastation.invalid>` (124 GB / 44 GA / 4 Build Barn). **All three were flipped to PRIVATE the same day** and the global git identity is now `Gloom <gloom@handofdevastation.invalid>` (neither the real name NOR the personal GitHub handle — see the privacy block in CLAUDE.md). Still owed before ANYTHING is published again: (a) strip the name from file content in all five repos — GloomsHub still has 48 mentions across 9 files, GloomsOverlays 5 across 3; (b) rewrite history to replace the old author email; (c) only then re-publish. See the privacy block in CLAUDE.md. ⚠ **ALSO: `GloomsHub` and `GloomsOverlays` have NO git remote and do not exist on GitHub yet.** GB and GA are live at `HandofDevastation/GloomsBars` / `GloomsAuras` (both now PRIVATE) and push normally; the other two are committed LOCALLY ONLY. The BigWigs packager → GitHub Releases → WoWup path in CLAUDE.md assumes all four exist, so creating those two repos + wiring `origin` is step zero of Phase G (or sooner, if remote backup matters). The `gh` CLI is authed as the owner's personal account, which is an org **admin**, so it can create them — but that handle must never land in a commit author or a file. |
