...
| **C** | Migrate Gloom's Bars as the proof (Bars tab, `/gb` reroute, toolkit → LibGloomSkin). | **IN PROGRESS — code built 2026-07-24, NOT QA'd** ← the owner runs the QA gate (in HANDOFF.md) next. Built: `LibGloomSkin-1.0` formalized (surface pinned CONTRACTS §4, incl. `RegisterWarmPairs`); GB hard-depends on the Hub, local toolkit + standalone window DELETED, whole editor mounts as the Bars tab (own footer row inside the tab), `/gb` + minimap button → `ToggleWindow("bars")`, warm pairs registered. GB's bar-ENGINE fonts deliberately stay on GB paths (CONTRACTS §1 note). Needs a FULL CLIENT RESTART (two addons changed + new Hub art files). |
...
- `Core.lua` (namespace, `GloomsHubDB`, ST copy-migration, dormant compat shim, `/gh` probe),
  ...
  `GloomsHub.toc` (Libs → Core → Skin → Shell → Media), `CLAUDE.md`, the 4 docs.
...
- **`~/GloomsBars` (Phase C, built not QA'd):** `## Dependencies: GloomsHub`; local toolkit
  ...
  `GB.COLOR` aliases the lib; `/gb` config branch + minimap button →
  `GloomsHub:ToggleWindow("bars")`; `/gb` diagnostics untouched; bar engine untouched.
