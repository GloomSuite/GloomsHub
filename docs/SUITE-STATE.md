...
| **G** | Packaging/release: `.pkgmeta`, `## Dependencies: GloomsHub`, embed LibGloomSkin, WoWup. | not started. ⚠ **PREREQUISITE (found 2026-07-24): `GloomsHub` and `GloomsOverlays` have NO git remote and do not exist on GitHub yet.** GB and GA are live at `HandofDevastation/GloomsBars` / `GloomsAuras` (both PUBLIC) and push normally; the other two are committed LOCALLY ONLY. The BigWigs packager → GitHub Releases → WoWup path in CLAUDE.md assumes all four exist, so creating those two repos + wiring `origin` is step zero of Phase G (or sooner, if remote backup matters). `gh` is authed as the org admin account, which is an **admin** of the HandofDevastation org, so it can create them. |
